...
angular.module('br').factory('cfgParser', function parseCfg() {
  ...
  function cloneMode(modeDef, modeName) {
    ...
    inhStack.shift();
    dir = mode.dir;
    mode.res = mode.res.map(function (path) {
      return dir + path
    });
    this.push(mode);
  }
  ...
  function inhPros(pros, target, optional) {
    pros.forEach(function (pro) {
      inheritPro(pro, target, optional);
    });
  }
...
angular.module('br').config(['imgFactoryProvider', function (provider) {
  provider.mobileRoute(window.app.cfg.isMobile);
}]);
angular.module('br').
...
angular.module('br').
  factory('glFactory', ['$rootScope', function ($rootScope) {
    ...
    glPort = bgl.pls(cvs, {
      ...
      normalLocation: {
        x: 0, get y() {
          ...
          return domEle.clientWidth
        }, get h() {
          return domEle.clientHeight - menu.clientHeight
        }
      }
    });
    ...
    return ex = {
      ...
      get lums() {
        return state.lums.map(function (n) {
          return n * 100
        });
      },
      set lums(v) {
        if ((v instanceof Array))
          state.lums = v.map(function (l) {
            return l / 100;
          });
        else if (!isNaN(v))
    ...
    function validImg(img) {
      return img instanceof Image && img.complete
    }
  }]);
angular.module('br').
  controller('multiController', ['$scope', 'glFactory', function ($scope, glFactory) {
    ...
    $scope.onchange = function (v) {
      ...
      v = parseInt(v);
      if (i === undefined) {
        $scope.value = (values = $scope.values = $scope.items.map(function () {
          return v
        }))[0];
