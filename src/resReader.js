...
function readDirFilesPromise(dirPath) {
  ...
  fs.readdir(dirPath, function (error, names) {
    return error ? defer.reject(error) : defer.resolve(names.map(function (name) {
      return dirPath + name
    }));
  });
...
module.exports = {
  ...
  readResSubDirs: function () {
    ...
    readDir(opt.resDir).then(function (files) {
      return Promise.all(files.map(function (name) {
        return asyncFS.statAsync(path.join(opt.resDir, name))
      })).then(function (states) {
        states.forEach(function (s, i) {
          if (s.isDirectory())dirs.push({name: files[i]});
        });
        return Promise.all(dirs.map(function (dir) {
          return asyncFS.readdirAsync(path.join(opt.resDir, dir.name))
        }));
      }).then(function (dirFiles) {
        dirFiles.forEach(function (files, i) {
          dirs[i].files = files;
        });
        defer.resolve(dirs);
      }, function (e) {
  ...
  readDirFiles: function (dir) {
    ...
    return readDir(dir).then(function (files) {
      return Promise.all(files.map(function (name) {
        return readFilePromise(path.join(dp, name))
      }));
    });
  }
};